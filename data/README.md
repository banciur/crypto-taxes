...
- `artifacts/accounts.json` is split into `real` and `artificial` lists. Real accounts are address-backed wallets used by source importers. Artificial accounts are manual ledger accounts exposed through the merged account catalog with `INTERNAL:<account_id>` ids and are not importer inputs.
