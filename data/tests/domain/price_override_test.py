...
from domain.ledger import AssetId, EventLocation, EventOrigin, LedgerEvent
...
from tests.helpers.ledger import make_leg, make_origin
...
def _event(external_id: str, *asset_ids: AssetId) -> LedgerEvent:
    return make_event(
        timestamp=_TS,
        event_origin=make_origin(external_id),
        legs=[make_leg(asset_id=asset, quantity=Decimal("1"), account_chain_id=LEDGER_WALLET) for asset in asset_ids],
    )
...
def _replacement(*sources: str) -> LedgerEvent:
    correction = LedgerCorrection(
        timestamp=_TS,
        sources=frozenset(make_origin(source) for source in sources),
        legs=frozenset([make_leg(asset_id=ETH, quantity=Decimal("1"), account_chain_id=KRAKEN_ACCOUNT_ID)]),
    )
...
def test_price_override_accepts_internal_synthetic_origin() -> None:
    ...
    override = _override(event_origin=make_origin("some-correction-uuid", location=EventLocation.INTERNAL))
...
def test_price_override_draft_has_no_id_and_override_generates_one() -> None:
    draft = PriceOverrideDraft(event_origin=make_origin("0xabc"), asset_id=ETH, rate_eur=Decimal("1500"))
    assert not hasattr(draft, "id")

    assert _override(event_origin=make_origin("0xabc")).id is not None
...
def test_override_is_orphaned_when_its_raw_event_is_folded_into_a_replacement() -> None:
    ...
    synthetic = _replacement("0xraw1", "0xraw2")
    override = _override(event_origin=make_origin("0xraw1"))
...
def test_origin_matching_no_corrected_event_is_a_problem() -> None:
    override = _override(event_origin=make_origin("0xghost"))
...
def test_problem_names_the_override_asset_and_targeted_origin() -> None:
    override = _override(event_origin=make_origin("0xghost"), asset_id=BTC)
...
def test_all_problems_are_reported_together() -> None:
    event = _event("0xpass", ETH)
    overrides = [
        _override(event_origin=make_origin("0xghost")),
