...
def test_weekly_tax_summary_skips_tax_free_disposals(
    ...
    projection = acquisition_disposal_projector.project(events=events)
    ...
    expected_taxable_links = {
        link.id for link in projection.disposal_links if link.lot_id == projection.acquisition_lots[1].id
    }
    assert taxable_link_ids == expected_taxable_links
    summaries = compute_weekly_tax_summary(tax_events, projection, events)
