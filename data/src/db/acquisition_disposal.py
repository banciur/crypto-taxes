...
class AcquisitionLotRepository:
    ...
    def list(self) -> list[AcquisitionLot]:
        orm_lots = self._session.query(AcquisitionLotOrm).all()
        return [self._lot_to_domain(lot) for lot in orm_lots]
...
class DisposalLinkRepository:
    ...
    def list(self) -> list[DisposalLink]:
        orm_links = self._session.query(DisposalLinkOrm).all()
        return [self._link_to_domain(link) for link in orm_links]
