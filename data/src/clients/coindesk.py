...
class CoinDeskAPIError(PriceClientError):
...
def fetch_histo_candles(
    *,
    ...
    """Fetch the single histo bucket covering ``timestamp``.

    Returns the candles and, when CoinDesk reports the requested window predates the
    instrument's first trade and the request is retried from that first-trade timestamp,
    the original requested timestamp to use as ``valid_from`` (else ``None``).
    """
    bucket_request = _resolve_bucket_request(aggregate_minutes)
    unix_ts = int(timestamp.timestamp())
    try:
        return _call_histo_api(
            client=client, market=market, instrument=instrument, to_ts=unix_ts, bucket=bucket_request
        ), None
    except CoinDeskAPIError as exc:
        adjusted = _maybe_adjust_to_first_trade(
            client=client, market=market, instrument=instrument, requested_ts=unix_ts, error=exc
        )
        if adjusted is None:
            raise
        adjusted_ts, override_dt = adjusted
        return _call_histo_api(
            client=client, market=market, instrument=instrument, to_ts=adjusted_ts, bucket=bucket_request
        ), override_dt
...
def fetch_spot_candle(
    *,
    ...
    entries, _ = fetch_histo_candles(
        client=client, market=market, instrument=instrument, timestamp=timestamp, aggregate_minutes=aggregate_minutes
    )
    if not entries:
...
def _call_histo_api(
    *, client: CoinDeskClient, market: str, instrument: str, to_ts: int, bucket: _BucketRequest
) -> list[SpotInstrumentOHLC]:
    if bucket.mode == "minute":
        return client.get_spot_historical_minutes(
            market=market,
            instrument=instrument,
            to_ts=to_ts,
            limit=1,
            aggregate=bucket.aggregate_units,
        )

    return client.get_spot_historical_hours(
        market=market,
        instrument=instrument,
        to_ts=to_ts,
        limit=1,
...
def _maybe_adjust_to_first_trade(
    *,
    ...
    metadata = client.get_market_instrument(market=market, instrument=instrument)
    if not metadata:
        return None
