...
- Accounts config entries now include `name` and `skip_sync`; `skip_sync=true` excludes that wallet from future fetches while retaining the wallet in tracked account metadata.
