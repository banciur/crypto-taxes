...
- Non-fee `EUR` is special. It is treated as an exact authoritative value already present in the event.
...
Non-fee `EUR` is projected only as valuation evidence. It does not become an `AcquisitionLot` or `DisposalLink`.
...
Valuation is event-aware. Pure market pricing is not enough on its own because the event may already contain exact `EUR`, the strongest available evidence may be the event itself, and some imported events are unbalanced because the system sees only owned wallets.

There are three acceptable sources of EUR value for non-fee projected assets:

1. exact non-fee `EUR` already present in the event
2. direct `PriceProvider.rate(asset_id, "EUR", timestamp)`
3. event-level remainder solving when exactly one distinct non-fee asset cannot be priced directly

Fees stay structurally separate from non-fee legs and do not participate in the non-fee balancing equation. A fee first tries to inherit the solved non-fee EUR-per-unit rate for the same asset in the same event. If that is not available, it falls back to direct pricing.
...
FIFO matches each projected disposal against older open acquisition lots of the same asset. Disposals are processed before acquisitions so that a same-event acquisition residual cannot fund a same-event disposal residual, and a single projected disposal leg may still produce multiple `DisposalLink`s, one for each historical lot fragment consumed by FIFO.
...
- Exact non-fee `EUR` is preserved as entered consideration rather than being replaced by market snapshots.
