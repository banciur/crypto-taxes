"use client";

import { useId } from "react";
import Image from "next/image";
import { OverlayTrigger, Tooltip } from "react-bootstrap";

import { CHAIN_METADATA, AVAILABLE_LOGOS } from "@/consts";

export type OriginIconProps = {
  place: string;
  size?: number;
  className?: string;
};

const formatOriginLabel = (originKey: string) =>
  `${originKey.charAt(0).toUpperCase()}${originKey.slice(1)}`;

export function OriginIcon({ place, size = 24, className }: OriginIconProps) {
  const hasLogo =
    Object.hasOwn(CHAIN_METADATA, place) || AVAILABLE_LOGOS.has(place);
  const label = hasLogo ? formatOriginLabel(place) : "Unknown";
  const src = hasLogo ? `/${place}-logo.svg` : "/question-mark.svg";

  return (
    <OverlayTrigger
      placement="top"
      overlay={<Tooltip id={useId()}>{place}</Tooltip>}
    >
      <Image
        src={src}
        alt={label}
        width={size}
        height={size}
        {...(className ? { className } : {})}
      />
    </OverlayTrigger>
  );
}
