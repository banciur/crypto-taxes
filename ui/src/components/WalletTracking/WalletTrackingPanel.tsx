...
function EventMarker({
  ...
  label: string;
  event: WalletTrackingState["lastAppliedEvent"];
}) {
  if (event === null) {
    return null;
  }
  ...
  return (
    <div className={`${styles.markerCard} p-3 h-100`}>
      <div className="small text-uppercase text-muted mb-1">{label}</div>
      <div className="small text-uppercase text-muted">{event.location}</div>
      <OriginId
        originId={event.externalId}
        place={event.location.toLowerCase()}
        className="small"
      />
    </div>
  );
...
export function WalletTrackingPanel({ state }: WalletTrackingPanelProps) {
  ...
  const markerCount =
    (state.lastAppliedEvent === null ? 0 : 1) +
    (state.failedEvent === null ? 0 : 1);
      ...
      <Row className="g-2 mb-4">
        <Col md={4}>
          <div className={styles.metricCard}>
            <div className={`text-uppercase text-muted ${styles.metricLabel}`}>
              Processed events
            </div>
            <div className={`fw-semibold ${styles.metricValue}`}>
      ...
      {markerCount > 0 && (
        <section className="mb-4">
          ...
          <Row className="g-2">
            <Col md={6}>
              <EventMarker
                label="Last applied event"
                event={state.lastAppliedEvent}
              />
            </Col>
            <Col md={6}>
              <EventMarker label="Failed event" event={state.failedEvent} />
            </Col>
          </Row>
        </section>
      )}
