...
- [ ] Refactor the event selection state in `ui/src/components/Events.tsx` from raw-specific naming to shared event-selection naming while keeping `eventOriginKey(eventOrigin)` as the canonical identity.
...
- [ ] Rename raw-specific selection props and callbacks in `ui/src/components/VirtualizedDateSections.tsx`, `ui/src/components/EventDateSection.tsx`, and `ui/src/components/LaneItem.tsx` to generic event-selection naming.
