...
- Modes: `FRESH` always fetches `(latest_cached_date - 1 day)` → now; `BUDGET` only fetches if the chain cache is not current through yesterday. Empty cache fetches all (no `from_date`).
...
- Persistence: SQLite `data/transactions_cache.db` via `TransactionsCacheRepository` with columns (`chain`, `hash`, `block_number`, `transaction_index`, `block_timestamp`, `payload JSON`); indexes on `(block_timestamp, block_number, transaction_index)`, `hash`, unique `(chain, hash)`.
